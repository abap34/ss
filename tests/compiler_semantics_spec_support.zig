...
pub fn expectObjectProperty(
    ...
    for (ir.nodes.items) |node| {
        ...
        const content = node.content orelse continue;
        if (!std.mem.eql(u8, content, expected_content)) continue;
        const value = ir.getNodeProperty(node.id, key) orelse continue;
        if (std.mem.eql(u8, value, expected_value)) return;
    }
