...
async function testLspFeatureSurface() {
  ...
  try {
    ...
    await withLspClient({ cwd: project }, async (client) => {
      ...
      assert(projectInfo.settings?.lsp?.completion === true, `feature surface projectInfo missing LSP settings: ${JSON.stringify(projectInfo)}`);
      assert(projectInfo.settings?.wysiwyg?.maxWaitMs === 700, `feature surface projectInfo missing WYSIWYG max wait: ${JSON.stringify(projectInfo)}`);
      assert(projectInfo.settings?.wysiwyg?.refreshAutomatically === true, `feature surface projectInfo missing WYSIWYG automatic refresh: ${JSON.stringify(projectInfo)}`);
