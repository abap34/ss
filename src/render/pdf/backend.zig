    ...
    switch (item) {
        ...
        .stroke_line => |value| {
            const line = c.SsPdfStrokeLine{
                ...
                .g = value.color.g,
                .b = value.color.b,
                .dash_on = value.dash_on,
                .dash_off = value.dash_off,
            };
            c.ss_pdf_stroke_line(pdf, &line);
        },
        .vector_path => |value| try replayVectorPath(allocator, pdf, value),
        .rounded_rect => |value| {
            const rect = c.SsPdfRoundedRect{
                .x = value.rect.x,
                .y = value.rect.y,
                .width = value.rect.width,
                .height = value.rect.height,
                .radius = value.radius,
                .has_fill = @intFromBool(value.fill != null),
                .fill_r = if (value.fill) |color| color.r else 0,
                .fill_g = if (value.fill) |color| color.g else 0,
                .fill_b = if (value.fill) |color| color.b else 0,
                .has_stroke = @intFromBool(value.stroke != null),
                .stroke_r = if (value.stroke) |color| color.r else 0,
                .stroke_g = if (value.stroke) |color| color.g else 0,
                .stroke_b = if (value.stroke) |color| color.b else 0,
                .line_width = value.line_width,
            };
            c.ss_pdf_fill_stroke_rounded_rect(pdf, &rect);
        },
