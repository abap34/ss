...
const primitive_descriptors = [_]PrimitiveDescriptor{
    ...
    .{ .op = .layout_v_all, .name = "layout_v_all", .min_arity = 1, .max_arity = 1, .arg_names = &.{"policy"}, .arg_sorts = &.{.string}, .result_sort = .document, .effect = .builds_graph, .summary = "Set the document default vertical fallback policy" },
