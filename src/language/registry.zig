...
const primitive_descriptors = [_]PrimitiveDescriptor{
    ...
    .{ .op = .new_page, .name = "new_page", .min_arity = 2, .max_arity = 2, .arg_names = &.{ "document", "title" }, .arg_sorts = &.{ .document, .string }, .result_sort = .page, .effect = .builds_graph, .summary = "Create a page in the scheduled document graph" },
    .{ .op = .new_object, .name = "new_object", .min_arity = 4, .max_arity = 4, .arg_names = &.{ "page", "content", "role_name", "payload_name" }, .arg_sorts = &.{ .page, .string, .string, .string }, .result_sort = .object, .effect = .builds_graph, .summary = "Create an object on an explicit page in the scheduled document graph" },
    .{ .op = .new_group, .name = "new_group", .min_arity = 2, .max_arity = 2, .arg_names = &.{ "page", "children" }, .arg_sorts = &.{ .page, .selection }, .result_sort = .object, .effect = .builds_graph, .summary = "Create a group on an explicit page in the scheduled document graph" },
