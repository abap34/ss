...
const primitive_descriptors = [_]PrimitiveDescriptor{
    ...
    .{ .op = .new_object, .name = "new_object", .min_arity = 4, .max_arity = 4, .arg_names = &.{ "page", "content", "role_name", "payload_name" }, .arg_sorts = &.{ .page, .string, .string, .string }, .result_sort = .object, .result_policy = .object_from_role_arg, .effects = &.{ .CreateNode, .WriteContent }, .summary = "Create an object on an explicit page in the scheduled document graph" },
