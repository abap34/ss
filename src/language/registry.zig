...
const primitive_descriptors = [_]PrimitiveDescriptor{
    ...
    .{ .op = .set_prop, .name = "set_prop", .min_arity = 3, .max_arity = 3, .arg_names = &.{ "target", "key", "value" }, .arg_types = &.{ Type.any, Type.string, Type.any }, .result_type = null, .result_policy = .target_arg, .summary = "Set, replace, or clear a property on a document, page, object, or object selection" },
