...
fn appendRoles(index: *DeclarationIndex, module_id: u32, class_name: []const u8, roles: []const []const u8) !void {
    for (roles) |role| {
...
fn appendFields(index: *DeclarationIndex, module_id: u32, class_name: []const u8, fields: []const ast.ObjectFieldDecl) !void {
    for (fields) |field| {
