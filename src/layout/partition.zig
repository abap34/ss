...
pub const Document = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, state: anytype) !Document {
        const builders = try allocator.alloc(PageBuilder, state.graph.page_order.items.len);
        ...
        for (state.graph.page_order.items, builders, 0..) |page_id, *builder, index| {
        ...
        for (result.pages, state.graph.page_order.items) |*page, page_id| page.* = .{ .page_id = page_id };
...
fn pageIndexForNode(state: anytype, pages: *const std.AutoHashMap(model.NodeId, usize), owners: *std.AutoHashMap(model.NodeId, ?usize), node_id: model.NodeId) !?usize {
    ...
    if (!entry.found_existing) {
        entry.value_ptr.* = if (state.layoutPageOf(node_id)) |page_id| pages.get(page_id) else null;
    }
